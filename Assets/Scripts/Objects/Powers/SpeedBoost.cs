...
{
    public class SpeedBoost : PowerTools
    {
        ...
        protected override void Action()
        {
            for (int i = 0; i < 2; i++)
            {
                _bodyMembersSmoke[i] = Instantiate(smokeEffect, _members[i].position, _members[i].rotation, _members[i]);
            }
        ...
        private IEnumerator EndPower()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                _bodyMembersSmoke[i].GetComponent<ParticleSystem>().Stop();
                Destroy(_bodyMembersSmoke[i],4);
            }
            while (time > 0)
            {
