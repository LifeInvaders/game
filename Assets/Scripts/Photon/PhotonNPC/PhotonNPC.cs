...
namespace People.NPC
{
    public class PhotonNPC : NPCdata
    {
        ...
        void Awake()
        {
            ...
            {
                NavMesh.SamplePosition(transform.position,out var warpPosition,10f, _agent.areaMask);
                _agent.transform.position = warpPosition.position;
                _agent.enabled = false;
                _agent.enabled = true;
            }
        }
        ...
        void Update()
        {
            if (PhotonNetwork.IsMasterClient && !_agent.pathPending && _agent.remainingDistance < 0.5f)
                GotoNextPoint();
